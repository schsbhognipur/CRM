    ...
    <div className="space-y-8 animate-fade-in max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
             ...
             <h1 className="text-3xl font-black text-slate-900  tracking-tight uppercase">Expense Terminals</h1>
          </div>
        ...
        <div className="flex justify-end gap-3 flex-wrap">
          <button 
            ...
            className="bg-white  text-slate-600  border-2 border-slate-200  px-6 py-3 rounded-2xl flex items-center gap-3 text-xs font-black uppercase tracking-widest hover:border-indigo-600 hover:text-indigo-600 transition-all shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {['This Month', 'Last Month', 'This Year'].map((label, i) => (
            <div key={label} className="bg-white  p-6 rounded-3xl shadow-sm border  relative overflow-hidden group">
               ...
               {(summary?.data?.byCategory || []).slice(0, 8).map((cat: any) => (
                 <div key={cat.categoryId} className="space-y-2">
                    <div className="flex justify-between items-end">
                       <p className="text-sm font-black uppercase text-slate-800 ">{cat.categoryName}</p>
      ...
      <div className="bg-white  rounded-[2.5rem] shadow-xl shadow-slate-200/50  border  overflow-hidden">
        ...
        <div className="p-6 border-b  flex flex-col md:flex-row gap-4 bg-slate-50  items-center">
          <div className="relative flex-1 group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-white  border-2 border-slate-100  focus:border-indigo-600 outline-none rounded-xl text-sm font-bold transition-all text-slate-900  placeholder:text-slate-400 focus:shadow-md"
              onChange={(e) => debouncedSearch(e.target.value)}
            />
          </div>
          <select 
             className="px-4 py-3 bg-white  border-2 border-slate-100  rounded-xl text-sm font-black uppercase tracking-widest outline-none text-slate-700  min-w-[200px]"
             value={categoryId}
          ...
          <div className="flex gap-2">
             <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-4 py-3 bg-white  border-2 border-slate-100  rounded-xl text-sm font-bold text-slate-700  outline-none" />
             <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="px-4 py-3 bg-white  border-2 border-slate-100  rounded-xl text-sm font-bold text-slate-700  outline-none" />
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 ">
               {isLoading ? (
                  ...
                  <tr>
                     <td colSpan={6} className="text-center p-12 text-xs font-black uppercase text-slate-400 tracking-widest hover:bg-slate-50  transition-colors">
                 ...
                 (filterData?.data?.transactions || []).map((t: any) => (
                    <tr key={t.id} className="hover:bg-slate-50  transition-colors group">
                       <td className="px-8 py-6 whitespace-nowrap">
                          <p className="text-xs font-black text-slate-800  uppercase">{new Date(t.transactionDate).toLocaleDateString()}</p>
                       ...
                       <td className="px-8 py-6">
                          <p className="text-xs font-black text-rose-600 bg-rose-50  px-3 py-1.5 rounded-lg border border-rose-100  uppercase tracking-widest w-max mb-2">
                             {t.expenseCategory?.name}
                          </p>
                          <p className="text-xs font-bold text-slate-600  max-w-sm truncate">{t.description}</p>
                       </td>
                       <td className="px-8 py-6 whitespace-nowrap">
                          <p className="text-sm font-black text-slate-900 ">₹{Number(t.amount).toLocaleString()}</p>
                       </td>
                       <td className="px-8 py-6 whitespace-nowrap">
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-700 ">{t.paymentMode}</p>
                       ...
                       <td className="px-8 py-6 whitespace-nowrap text-right">
                          <div className="flex justify-end gap-1 opacity-50 group-hover:opacity-100 transition-opacity">
                             {canEdit(t) && (
                                <button onClick={() => handleEdit(t)} className="w-8 h-8 rounded-xl bg-slate-100  text-slate-500 hover:text-indigo-600 hover:bg-indigo-50  flex items-center justify-center transition-all" title="Edit Expense">
                                   <Edit2 size={14} />
                                </button>
                             )}
                             <button onClick={() => handlePrint(t.id)} className="w-8 h-8 rounded-xl bg-slate-100  text-slate-500 hover:text-emerald-600 hover:bg-emerald-50  flex items-center justify-center transition-all" title="Print Voucher">
                                <Printer size={14} />
                             </button>
                             {isAdmin && (
                                <button onClick={() => handleDelete(t.id)} className="w-8 h-8 rounded-xl bg-slate-100  text-slate-500 hover:text-rose-600 hover:bg-rose-50  flex items-center justify-center transition-all" title="Void Transaction">
                                   <Trash2 size={14} />
                                </button>
                             )}
                          </div>
        ...
        {!isLoading && filterData?.data?.meta && filterData?.data?.meta?.totalPages > 1 && (
           <div className="px-8 py-6 border-t  bg-slate-50  flex justify-between items-center text-xs font-black uppercase tracking-widest text-slate-400">
              <p>Showing Page {page} of {filterData?.data?.meta?.totalPages} ({filterData?.data?.meta?.total} Disbursals)</p>
              <div className="flex gap-2">
                 <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-4 py-2 bg-white  rounded-lg hover:text-indigo-600 disabled:opacity-50 transition-colors">Prev</button>
                 <button disabled={page === filterData?.data?.meta?.totalPages} onClick={() => setPage(p => p + 1)} className="px-4 py-2 bg-white  rounded-lg hover:text-indigo-600 disabled:opacity-50 transition-colors">Next</button>
              </div>
           </div>
        )}
      </div>
