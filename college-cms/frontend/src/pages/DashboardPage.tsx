    ...
    <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard 
          ...
          subtext={`+${studentStats.new_this_month} new`} 
        ...
        <KPICard 
          ...
          subtext={`${feeStats.collectionRate || 0}% target`} 
        ...
        <KPICard 
          title="Surplus" 
          value={`₹${((financeStats.monthCredit || 0) - (financeStats.monthDebit || 0)).toLocaleString('en-IN')}`} 
          subtext="Net Monthly" 
          icon={<CreditCard size={18} className="text-rose-600" />}
          gradient="from-rose-500/5 to-transparent"
          trend={(financeStats.monthCredit || 0) >= (financeStats.monthDebit || 0) ? 'up' : 'down'}
        />
      </div>
                ...
                <div key={log.id} className="flex gap-3">
                   ...
                   <div className="min-w-0 flex-1">
                      <div className="flex justify-between items-start">
                         <p className="text-[11px] font-black text-slate-800 dark:text-white truncate">{log.user?.name?.split(' ')[0] || 'System'}</p>
      ...
      <div className="bg-slate-900 dark:bg-slate-800/50 p-6 rounded-[2rem] text-white relative overflow-hidden">
         ...
         <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
               {(summary?.recentTransactions || []).slice(0, 3).map((tx: any, i: number) => (
                 <div key={tx.id} className="p-4 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all group">
                    ...
                    <div className="flex justify-between items-center">
                       <div className="min-w-0 flex-1">
                          <p className="text-xs font-black truncate">{tx.student?.name || tx.description || 'System Entry'}</p>
                          <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">{(tx.subType || 'OTHER').replace(/_/g, ' ')}</p>
                       </div>
               ...
               {(!summary?.recentTransactions || summary.recentTransactions.length === 0) && (
                 <div className="col-span-full py-8 text-center text-slate-500 text-[10px] font-black uppercase tracking-widest bg-white/5 rounded-2xl border border-dashed border-white/10">
...
const KPICard = ({ title, value, subtext, icon, gradient, trend }: any) => (
  <div className="relative bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-lg shadow-slate-100 dark:shadow-none border dark:border-slate-700 overflow-hidden group">
     <div className={clsx("absolute inset-0 bg-gradient-to-br transition-opacity transition-duration-500", gradient)} />
