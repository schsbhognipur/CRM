...
const ReportsPage = () => {
   ...
   return (
      <div className="space-y-8 animate-fade-in">
         <div className="">
            <h1 className="text-3xl font-black text-slate-900  mb-1">Intelligence Reports</h1>
            ...
            <div className="w-full xl:w-96 space-y-4">
               {reports.map((r, i) => (
                  <motion.button
                     ...
                     className={clsx(
                        "w-full p-6 bg-white  rounded-[2rem] text-left transition-all relative overflow-hidden group border ",
                        activeReport === r.id
                           ? "border-indigo-600 shadow-2xl shadow-indigo-600/20 ring-4 ring-indigo-50 "
                  ...
                  >
                     <div className="flex items-center gap-5 relative z-10">
                        <div className={clsx(
                           ...
                           activeReport === r.id ? "bg-indigo-600 text-white shadow-xl shadow-indigo-600/40" : "bg-slate-50  text-slate-400 group-hover:text-indigo-600"
                        )}>
                           <r.icon size={26} />
                        </div>
                        <div>
                           <p className={clsx("font-black text-sm uppercase tracking-tight", activeReport === r.id ? "text-indigo-600" : "text-slate-600 ")}>{r.name}</p>
               ...
               <AnimatePresence mode="wait">
                  {!activeReport ? (
                     <motion.div
                        ...
                        className="h-[600px] flex flex-col items-center justify-center p-20 text-center bg-white  rounded-[3rem] border-2 border-dashed border-slate-100  shadow-xl shadow-slate-200/50 "
                     >
                        <div className="w-24 h-24 bg-slate-50  rounded-[2rem] flex items-center justify-center text-slate-200  mb-8">
                           <TableIcon size={48} />
                        </div>
                        <h3 className="text-2xl font-black text-slate-900  mb-2 uppercase tracking-tight">Select Intelligence Module</h3>
                     ...
                     >
                        ...
                        {/* Control Interceptor */}
                        <div className="bg-white  p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50  border  flex flex-wrap gap-8 items-end">
                                       ...
                                       {activeReport === 'daybook' && reportData && ([...(reportData.credits || []), ...(reportData.debits || [])]).map((row: any) => (
                                          <tr key={row.id} className="hover:bg-slate-50  transition-colors cursor-pointer group">
                                             <td className="px-8 py-6">
                                                <div className="text-sm font-black text-slate-800  uppercase truncate max-w-[200px]">{row.student?.name || row.description}</div>
                                             ...
                                             <td className="px-8 py-6">
                                                <span className="px-3 py-1 bg-slate-100  rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest">{row.paymentMode}</span>
                                             </td>
                                             <td className="px-8 py-6 text-right">
                                                <div className="text-sm font-bold text-slate-600 ">{new Date(row.transactionDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
...
const ReportCard = ({ label, value, color }: any) => (
   <div className="bg-white  p-6 rounded-[1.5rem] border  shadow-sm relative overflow-hidden group hover:border-indigo-500 transition-all">
