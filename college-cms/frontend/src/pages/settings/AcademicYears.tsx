    ...
    <div className="space-y-8 animate-fade-in max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
             ...
             <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Academic Timeline</h1>
          </div>
      ...
      {isLoading ? (
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {years.map((y: any) => (
               <div key={y.id} className={clsx("p-6 rounded-[2rem] border-2 transition-all relative overflow-hidden group", y.isActive ? "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500 shadow-xl shadow-emerald-500/10" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700")}>
                  ...
                  <div className="flex justify-between items-start relative z-10">
                     <div>
                        <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white">{y.label}</h3>
                        <p className="text-xs font-bold text-slate-500 mt-1">
                           {new Date(y.startDate).toLocaleDateString()} — {new Date(y.endDate).toLocaleDateString()}
                        </p>
                     </div>
                     {y.isActive ? (
                        <div className="flex items-center gap-2 bg-emerald-100 text-emerald-700 dark:bg-emerald-800 dark:text-emerald-100 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-emerald-200 dark:border-emerald-700">
                           <CheckCircle2 size={14} /> Active
                        </div>
                     ) : (
                        <button onClick={() => openModal(y)} className="p-2 text-slate-400 hover:text-indigo-600 bg-slate-100 dark:bg-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-all">
                           <Edit2 size={16} />
                        </button>
                     )}
                  </div>
                  ...
                  {!y.isActive && (
                     <button 
                        ...
                        className="mt-6 w-full py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:border-indigo-600 hover:text-indigo-600 transition-all group-hover:bg-white dark:group-hover:bg-slate-900"
                     >
      ...
      <AnimatePresence>
         {isModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
               ...
               <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full">
                  <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white mb-6">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                     <div>
                        ...
                        <input {...register('label')} placeholder="e.g. 2025-26" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-600 rounded-xl font-bold text-sm outline-none transition-all" />
                     ...
                     <div>
                        ...
                        <input type="date" {...register('startDate')} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-600 rounded-xl font-bold text-sm outline-none transition-all" />
                     ...
                     <div>
                        ...
                        <input type="date" {...register('endDate')} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-indigo-600 rounded-xl font-bold text-sm outline-none transition-all" />
                        {errors.endDate && <p className="text-[10px] text-rose-500 font-bold ml-1 mt-1">{errors.endDate.message}</p>}
                     </div>
                     <div className="flex gap-2 pt-4">
                        <button type="button" onClick={closeModal} className="flex-1 py-3 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-slate-200 transition-colors">Abort</button>
