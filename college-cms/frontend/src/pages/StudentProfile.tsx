...
const StudentProfile = () => {
   ...
   const [receiptTheme, setReceiptTheme] = useState<'modern' | 'classic'>('modern');
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
         ...
         <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
               <button
                  ...
                  className="px-4 py-2 bg-white  border  rounded-xl text-xs font-bold uppercase tracking-widest shadow-sm hover:shadow-md transition-all"
               >
                  Edit Identity
               </button>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1 space-y-8">
               <div className="bg-white  rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50  border  text-center relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                     ...
                     <h2 className="text-2xl font-black text-slate-900  mb-1 uppercase tracking-tight">{student.name}</h2>
                     ...
                     <div className="flex flex-col gap-3">
                        <div className="bg-slate-50  p-4 rounded-2xl border  text-left">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <p className="text-sm font-bold text-slate-700 ">{student.status}</p>
                           </div>
                        </div>
                        <div className="bg-slate-50  p-4 rounded-2xl border  text-left">
                           <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Academic Unit</p>
                           <p className="text-sm font-bold text-slate-700 ">{student.course.name}</p>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <div className="lg:col-span-3 space-y-8">
               {/* Tabs */}
               <div className="flex p-1.5 bg-white  rounded-2xl border  w-fit">
                  {tabs.map(tab => (
                     <button
                        key={tab}
                     ...
                     {activeTab === 'Fee History' && (
                        <div className="space-y-6">
                           {student.fees.map((fee: any) => (
                              <div key={fee.id} className="bg-white  p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50  border  flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-indigo-500 transition-all">
                                 <div>
                                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1">Academic Year {fee.academicYear?.label || 'N/A'}</p>
                                    <h3 className="text-xl font-black text-slate-900  mb-2">Year {fee.yearOfStudy} Curricular Fees</h3>
                                    <div className="flex items-center gap-2">
                                       <div className="px-2 py-0.5 bg-slate-100  rounded text-[10px] font-black uppercase text-slate-500">{fee.feeStructure?.name || 'Standard Structure'}</div>
                                    </div>
                                 </div>
                                 <div className="flex gap-12">
                                    <div className="text-center">
                                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total</p>
                                       <p className="text-lg font-black text-slate-800 ">₹{fee.totalAmount.toLocaleString()}</p>
                                    </div>
                                    <div className="text-center">
                     ...
                     {activeTab === 'Transaction History' && (
                        <div className="bg-white  rounded-[2.5rem] shadow-xl shadow-slate-200/50  border  overflow-hidden">
                           <div className="p-8 border-b  flex justify-between items-center">
                              <h3 className="text-xl font-black text-slate-900 ">Financial Ledger</h3>
                                 ...
                                 <tbody className="divide-y divide-slate-100 ">
                                    {student.transactions?.map((t: any) => (
                                       <tr key={t.id} className="hover:bg-slate-50  transition-colors">
                                          <td className="px-8 py-5 whitespace-nowrap text-sm font-bold text-slate-600 ">{new Date(t.transactionDate).toLocaleDateString()}</td>
                                          ...
                                          <td className="px-8 py-5 whitespace-nowrap text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-100/50  w-fit rounded-lg self-center">{t.paymentMode}</td>
                                          <td className="px-8 py-5 whitespace-nowrap text-right">
                                             <button onClick={() => downloadReceipt(t.id)} className="w-10 h-10 bg-slate-50  rounded-xl text-slate-400 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center mx-auto lg:ml-auto">
                                                <Download size={18} />
                                             </button>
                                          </td>
                                       </tr>
...
const InfoSection = ({ title, icon, children }: any) => (
   <div className="bg-white  p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50  border ">
...
const DetailRow = ({ label, value }: any) => (
   <div className="flex justify-between items-center group">
      ...
      <span className="text-sm font-black text-slate-800  group-hover:text-indigo-600 transition-colors">{value}</span>
   </div>
);
