    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
           <button className="bg-white  border  px-4 py-2 rounded-xl text-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-1 md:col-span-2 bg-white  rounded-2xl p-6 shadow-sm border ">
               <h3 className="text-lg font-bold mb-6">Recent Expenses</h3>
               <div className="space-y-4 max-h-[400px] overflow-auto pr-2">
                 {(expenses?.transactions || []).map((ex: any) => (
                   <div key={ex.id} className="flex justify-between items-center p-3 hover:bg-gray-50  rounded-xl transition">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 bg-red-50  text-red-600 rounded-full flex items-center justify-center">
                            <Receipt size={20} />
                         </div>
                         <div>
                            <p className="font-bold text-sm text-gray-900  uppercase">{ex.description}</p>
...
const StatCard = ({ title, amount, change, color }: any) => (
  <div className="bg-white  p-6 rounded-2xl shadow-sm border ">
    ...
    <div className="flex items-end justify-between">
       ...
       <span className="text-xs font-bold text-green-500 bg-green-50  px-2 py-1 rounded-full">{change}</span>
    </div>
  </div>
);
